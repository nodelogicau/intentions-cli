#!/bin/sh
# intentions installer — https://github.com/nodelogicau/intentions-cli
#
...
#   INTENTIONS_VERSION      tag to install, with or without a leading "v", or "latest" (default: latest release)
#   INTENTIONS_INSTALL_DIR  directory to install into (default: see choose_dir)
#   INTENTIONS_REPO         owner/name to download from (default: nodelogicau/intentions-cli)
#
...
REPO="${INTENTIONS_REPO:-nodelogicau/intentions-cli}"
BIN="intentions"
...
TAG="${INTENTIONS_VERSION:-}"
[ "$TAG" = latest ] && TAG=""
if [ -z "$TAG" ]; then
  TAG="$(latest_tag)" || true
  [ -n "$TAG" ] || die "could not determine the latest release; set INTENTIONS_VERSION"
fi
...
TMP="$(mktemp -d 2>/dev/null || mktemp -d -t intentions)"
...
# Extract in the temp dir, never the cwd (a workspace checkout has an intentions/ directory).
tar -xzf "${TMP}/${ARCHIVE}" -C "$TMP" "$BIN" || die "could not extract ${BIN} from ${ARCHIVE}"

choose_dir() {
  if [ -n "${INTENTIONS_INSTALL_DIR:-}" ]; then
    mkdir -p "$INTENTIONS_INSTALL_DIR" || die "cannot create ${INTENTIONS_INSTALL_DIR}"
    printf '%s' "$INTENTIONS_INSTALL_DIR"; return
  fi
