...
var presets = []preset{
	{"claude", ".claude/skills/intentions/SKILL.md", ".claude/skills/intentions/SKILL.md", kindSkillFile, "Claude Code (also read by GitHub Copilot)"},
	{"copilot", ".github/skills/intentions/SKILL.md", ".copilot/skills/intentions/SKILL.md", kindSkillFile, "GitHub Copilot"},
	{"agents", ".agents/skills/intentions/SKILL.md", ".agents/skills/intentions/SKILL.md", kindSkillFile, "vendor-neutral Agent Skills location (read by Copilot)"},
	{"cursor", ".cursor/rules/intentions.mdc", "", kindCursorRule, "Cursor project rule"},
...
var copilotLocations = []string{".claude/skills/intentions/SKILL.md", ".github/skills/intentions/SKILL.md", ".agents/skills/intentions/SKILL.md"}
...
Harness presets (--harness):
  claude     .claude/skills/intentions/SKILL.md   (default; Claude Code, also read by Copilot)
  copilot    .github/skills/intentions/SKILL.md   (GitHub Copilot)
  agents     .agents/skills/intentions/SKILL.md   (vendor-neutral; read by Copilot)
  cursor     .cursor/rules/intentions.mdc         (Cursor rule)
...
func (a *app) skillInstallCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Writes the skill for each selected preset (see 'intentions skill --help').
...
version — use it in CI.`,
		...
		RunE: a.run(func(cmd *cobra.Command, args []string) error {
			...
			if err := a.emit(out, func(w io.Writer) {
				for _, r := range results {
					...
					case r["created"] == true:
						fmt.Fprintf(w, "Installed skill (intentions %s, %s) at %s\n", v, r["harness"], r["path"])
					case r["updated"] == true:
						fmt.Fprintf(w, "Updated skill (intentions %s, %s) at %s\n", v, r["harness"], r["path"])
					default:
	...
	cmd.Flags().BoolVar(&force, "force", false, "replace a skill file not written by intentions")
...
func installTarget(t target, force bool) (string, error) {
	...
	case kindAgentsSection:
		if exists {
			...
			if broken {
				return "", &ExitError{Code: apperr.ExitRuntime, ErrCode: "conflict", Err: fmt.Errorf("%s: %v", t.path, skill.ErrBrokenSection)}
			}
	...
	default:
		...
		if exists && !skill.HasMarker(existing) && !force {
			return "", &ExitError{Code: apperr.ExitRuntime, ErrCode: "conflict", Err: fmt.Errorf("%s exists and was not written by intentions; pass --force to replace it", t.path)}
		}
		next = rendered
	}
...
func duplicateWarnings(t target) []string {
	...
	return []string{fmt.Sprintf("GitHub Copilot also reads %s; it will load the intentions skill more than once — keep one of %s", strings.Join(others, " and "), strings.Join(copilotLocations, ", "))}
}
