BINARY  := intentions
MODULE  := github.com/nodelogicau/intentions-cli
...
build:
	go build -trimpath -ldflags '$(LDFLAGS)' -o $(DIST)/$(BINARY) ./cmd/intentions
...
cross:
	@for p in $(PLATFORMS); do \
		...
		echo "  $$out"; \
		GOOS=$$os GOARCH=$$arch go build -trimpath -ldflags '$(LDFLAGS)' -o $$out ./cmd/intentions || exit 1; \
	done
