// Package intentionsskill embeds the agent-facing skill so the binary can
...
var (
	...
	markerLine = regexp.MustCompile(`(?m)^<!-- installed by intentions [^;\n]*; regenerate with: intentions skill install[^>\n]*-->\n?`)
	// sectionStart / sectionEnd bound the region owned inside AGENTS.md.
	sectionStart = regexp.MustCompile(`(?m)^<!-- intentions:skill:start — installed by intentions [^;\n]*; regenerate with: intentions skill install --harness agents-md -->\n`)
	sectionEnd   = regexp.MustCompile(`(?m)^<!-- intentions:skill:end -->\n?`)
	headingLine  = regexp.MustCompile(`^(#{1,5}) `)
)

// ErrBrokenSection is returned when a start marker has no matching end marker.
var ErrBrokenSection = errors.New("AGENTS.md has an intentions start marker but no end marker; fix or remove it by hand")
...
func markerFor(version, hint string) string {
	return "<!-- installed by intentions " + NormaliseVersion(version) + "; regenerate with: intentions skill install" + hint + " -->"
}

// SectionStart is the opening marker of the AGENTS.md section.
func SectionStart(version string) string {
	return "<!-- intentions:skill:start — installed by intentions " + NormaliseVersion(version) + "; regenerate with: intentions skill install --harness agents-md -->"
}

// SectionEnd is the closing marker of the AGENTS.md section.
const SectionEnd = "<!-- intentions:skill:end -->"
...
func RenderAgentsSection(version string) []byte {
	...
	b.WriteString("\n## intentions — planning with intentions\n\n")
