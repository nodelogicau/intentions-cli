...
func TestRenderStampsVersionAndMarker(t *testing.T) {
	...
	if !strings.Contains(string(head), "name: intentions") {
...
func TestCursorRule(t *testing.T) {
	...
	want := "---\ndescription: \"" + Description() + "\"\nalwaysApply: false\n---\n<!-- installed by intentions 1.2.3; regenerate with: intentions skill install --harness cursor -->\n\nYou are drafting a person's plans"
	...
	if strings.Contains(out, "name: intentions") {
...
func TestAgentsSectionAndSplice(t *testing.T) {
	...
	if !strings.Contains(s, "intentions workspace --json                  # which workspace, found how (exit 5 = none)") {
	...
	if !strings.HasPrefix(string(out), above) || !strings.HasSuffix(string(out), below) || !strings.Contains(string(out), "installed by intentions 2.0.0") || strings.Contains(string(out), "0.1.0") {
		t.Errorf("replace:\n%s", out)
	}
