...
`select` SHALL refuse a placed intention unless `--replace` is given, in which case the previous placement is cleared and the new one written in the same act with a new RESOLUTION record; the previous record is left as it is.
