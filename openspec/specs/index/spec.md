...
`index.yaml` SHALL carry `format: intentions/0.1` and `entries`, one per object file in every type directory, each with `id`, `type` (`desire`, `intention`, `availability`, `commitment`, `resolution`), `subject` (omitted for commitments and resolutions), `path` (relative to the workspace root), `version`, `retired` (the kind, when present), and `refs` (the sorted outbound ids from `serves`, `window.relative.target`, `retired.superseded_by`, `retired.adopted_as`, `intention`, `origin.resolution`, `displaced`). Entries SHALL be sorted by id. Nothing not derivable from the file SHALL appear.
