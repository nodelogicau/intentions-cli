...
A calendar anchor SHALL be one of: a year `YYYY`; a month `YYYY-MM`; an ISO week `YYYY-Www`; a day `YYYY-MM-DD`; a season `YYYY-21` to `YYYY-24`; a quarter `YYYY-33` to `YYYY-36`; a bounded interval `A/B` between any two of these with A not after B; an open interval `../B` or `A/..`. Qualifiers `?`, `~`, `%` SHALL be refused. Normalisation SHALL upper-case `W`, zero-pad fields, and collapse `A/A` to `A`.
