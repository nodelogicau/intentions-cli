...
Every write SHALL store the computed version in the file under `version`. The computed value is authoritative. `show --json` SHALL report `version` as computed and `version_cached` when the file's value differs.
...
- **THEN** the rewritten file's `version` equals the newly computed value
